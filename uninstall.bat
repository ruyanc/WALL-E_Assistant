@echo off
REM ===========================================================
REM  WALL-E 桌面宠物 卸载脚本
REM  用法：uninstall.bat  或  uninstall.bat silent（不暂停）
REM ===========================================================
setlocal
chcp 65001 >nul

set "SILENT=0"
if /i "%~1"=="silent" set "SILENT=1"

echo 正在关闭正在运行的 WALL-E ...
taskkill /IM "WALL-E.exe" /F >nul 2>nul

set "INSTALL_DIR=%LOCALAPPDATA%\Programs\WALL-E"
set "DESKTOP=%USERPROFILE%\Desktop"
set "STARTUP=%APPDATA%\Microsoft\Windows\Start Menu\Programs\Startup"

echo 删除程序文件（含安装目录内的操作手册）...
if exist "%INSTALL_DIR%" rmdir /S /Q "%INSTALL_DIR%"

echo 删除快捷方式...
if exist "%DESKTOP%\WALL-E Assistant.lnk" del /F /Q "%DESKTOP%\WALL-E Assistant.lnk"
if exist "%STARTUP%\WALL-E Assistant.lnk" del /F /Q "%STARTUP%\WALL-E Assistant.lnk"
if exist "%DESKTOP%\WALL-E 桌面宠物.lnk" del /F /Q "%DESKTOP%\WALL-E 桌面宠物.lnk"
if exist "%STARTUP%\WALL-E 桌面宠物.lnk" del /F /Q "%STARTUP%\WALL-E 桌面宠物.lnk"

echo.
echo 卸载完成。
echo 提示：你的待办、记事本、提醒与设置仍保存在 "%APPDATA%\WALL-E"
echo 如需彻底清除，请手动删除该文件夹。
if not "%SILENT%"=="1" pause
endlocal
