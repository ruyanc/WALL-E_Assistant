...
class WalleSprite(Image):
    ...
    def _reload_anim(self, name: str) -> None:
        ...
        self._fps = max(1, int(spec.get("fps", 4)))
        self._frame_idx = 0
        self._current_path = ""
        self.pause()
        self._show_frame()
        if not self._paused and self.parent is not None:
            self.resume()
    ...
    def resume(self) -> None:
        ...
        if self._ev is not None:
            return
        self._ev = Clock.schedule_interval(self._tick, 1.0 / self._fps)
